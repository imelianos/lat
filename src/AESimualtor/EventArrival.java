...
public class EventArrival extends Event{
	...
	public static double meanArrivalStallTime=1.0;
		
	EntityPatient patient;
	
	public EventArrival(EntityPatient patient){
	...
	void execute(AbstractSimulator simulator) {
		...
		if (patient.getNextState() == Constants.PatientStates.ARRIVAL.ordinal()) {
			...
			void executeArriv(Simulator simulator, EntityPatient patient){
								
				patient.addToStagesPassed(patient.getCurrentState());

				patient.setNextState(patient.getRandomNextState(patient.getCurrentState()));
				
				time = ((Simulator) simulator).now()+Random.exponential(EventArrival.meanArrivalStallTime);
